    ...
    public void query() {
        ...
        Integer i1 =
                jdbcTemplate.queryForObject(
                        "select count(*) from sys_user where username='jasper'", Integer.class);
        log.info("count={}", i1);
        String username =
                jdbcTemplate.queryForObject(
                        "select username from sys_user where username='jasper'", String.class);
        log.info("username={}", username);

        SysUserDTO sysUserDTO1 =
                jdbcTemplate.queryForObject(
        ...
        RowMapper<SysUserDTO> rowMapper =
                (rs, rowNum) -> {
                    SysUserDTO sysUserDTO = new SysUserDTO();
                    sysUserDTO.setId(rs.getLong("id"));
                    sysUserDTO.setUsername(rs.getString("username"));
        ...
        List<SysUserDTO> list =
                jdbcTemplate.query("select id,username,password from sys_user", rowMapper);
        log.info("list={}", list);
    }
}
